...
fn image_to_base64(file_type: &str) {
    ...
    let base64 = image_base64::to_base64(
        &format!("res/{}.{}", FILE_NAME, file_type),
        image_base64::FileType::from(file_type),
    );
...
fn base64_to_image(file_type: &str) {
    ...
    let mut output =
        File::create(&Path::new(&format!("output/{}.{}", FILE_NAME, file_type))).unwrap();
    output.write_all(img.unwrap().as_slice()).unwrap();
}

fn validate(file_type: &str) {
    assert_eq!(
        get_file_size("res", file_type),
